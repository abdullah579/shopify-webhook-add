class PartyButton extends HTMLButtonElement {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Redeem Button Onclick
    // redeemPointEventListeners();

    let elementss = document.getElementById('test-btn');

    if (typeof(elementss) != 'undefined' && elementss != null)
    {
      console.log("OK");
    }else{
      console.log("NOT OK")
    ...
    function clickkdddkd(){
      document.getElementById('redeem-point-aaaa').addEventListener('click', function(){
        console.log("HOIDSE");
      });
    }
    ...
    function attachRedeemContentEventListeners() {
      document.getElementById('back-button-in-redeemSDfggh').addEventListener('click', showMainContent);
    }
  ...
  function createDiscount(points){
    const param = 'event=createDiscount&points=' + points;
    fetch('/apps/conn-user?' + param, {
      headers: {
        'ngrok-skip-browser-warning': 'true'
      }
